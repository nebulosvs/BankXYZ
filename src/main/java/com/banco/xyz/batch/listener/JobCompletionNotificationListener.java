...
public class JobCompletionNotificationListener implements JobExecutionListener {
	...
	public void beforeJob(JobExecution jobExecution) {
		...
		log.info("Inicio del trabajo batch con ID: {} job={}", jobExecution.getId(),
	...
	public void afterJob(JobExecution jobExecution) {
		...
		log.info("Job completado con ID: {} status={}", jobExecution.getId(), jobExecution.getStatus());
		log.info("Resumen del Job: {}", jobExecution);
		jobExecution.getStepExecutions().forEach(step -> log.info(
				"[PERF-JOB] step={} read={} write={} skip={} rollback={} commit={}",
		...
		switch (jobName) {
			...
			default -> log.info("Sin resumen de negocio para {}", jobName);
		}
	}
	...
	private void resumenTransacciones() {
		...
		jdbcTemplate.query(
				"SELECT id, fecha, monto, tipo, anomalia, observacion FROM transacciones_diarias ORDER BY id",
				rs -> {
	...
	private void resumenIntereses() {
		...
		jdbcTemplate.query(
				"SELECT cuenta_id, nombre, tipo, saldo_inicial, interes_calculado, saldo_final FROM cuentas_con_interes ORDER BY id",
				rs -> {
